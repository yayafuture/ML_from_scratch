...
class XGBoostRegressionTree(DecisionTree):
    ...
    def _approximate_update(self, y):
        # get vector of scores on leaves
        y, y_pred = self._split(y)
...
class XGBoost(object):
    ...
    def __init__(self, n_estimators=200, learning_rate=0.01, min_samples_split=2,
        ...
        self.trees = [XGBoostRegressionTree(
                min_samples_split=self.min_samples_split,
                min_impurity=self.min_impurity,
                max_depth=self.max_depth,
                loss=self.loss) for _ in range(n_estimators)]
    ...
    def fit(self, X, y):
        ...
        for i in self.bar(range(self.n_estimators)):
            tree = self.trees[i]
            # this serves as Y, in each iteration, we will update only y_pred
            y_and_pred = np.concatenate((y, y_pred), axis=1)
            tree.fit(X, y_and_pred)
            update_pred = np.reshape(tree.predict(X), (m, -1))
            y_pred += update_pred
    ...
    def predict(self, X):
        ...
        for tree in self.trees:
            ...
            update_pred = np.reshape(tree.predict(X), (m, -1))
            if y_pred is None: # y_pred is np array
